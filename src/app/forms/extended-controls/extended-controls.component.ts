...
export class ExtendedControlsComponent implements OnInit {
  ...
  @ViewChild("skillsAutocomplete") matAutocomplete: MatAutocomplete;
  ...
  ngOnInit() {
    this.filteredSkills = this.controlsForm.controls.skills.valueChanges.pipe(
      startWith(null as string),
      map((skill: string | null) =>
        skill ? this._filterSkills(skill) : this.prefilledSkills.slice()
      )
    );
  }
  ...
  private _filterSkills(value: string): string[] {
    ...
    return this.prefilledSkills.filter(
