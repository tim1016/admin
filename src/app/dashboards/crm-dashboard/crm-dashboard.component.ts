...
export class CrmDashboardComponent implements OnInit {
  ...
  leadsTableDisplayedColumns: string[] = [
    "name",
    "email",
    "source",
    "mobile",
  ...
  constructor(
    ...
    this.leadsTableDataSource = new MatTableDataSource(
      route.snapshot.data["data"].leadsData
    );
    // tslint:disable-next-line:no-string-literal
    ...
    this.completedMarketingTasks = this.getCompletedTasks(
      this.marketingTasksBoardForm
    );
    ...
    this.salesTasksBoardForm.valueChanges.subscribe((val) => {
      this.completedSalesTasks = this.getCompletedTasks(
        this.salesTasksBoardForm
      );
    });
    ...
    this.completedCustomerSupportTasks = this.getCompletedTasks(
      this.customerSupportTasksBoardForm
    );
  ...
  ngOnInit() {
    ...
    this.leadsTableDataSource.sortingDataAccessor = (item, property) => {
      switch (property) {
        case "last_contact":
          return new Date(item.last_contact);
        default:
