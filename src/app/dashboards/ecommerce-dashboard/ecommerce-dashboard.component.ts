...
export class EcommerceDashboardComponent implements OnInit {
  ...
  recentOrdersTableDisplayedColumns: string[] = [
    "id",
    "product",
    "category",
    "location",
  ...
  latestTicketsTableDisplayedColumns: string[] = [
    "name",
    "ticket_id",
    "subject",
    "phone",
  ...
  @ViewChild("latestTicketsPaginator", { static: true })
  latestTicketsPaginator: MatPaginator;
  ...
  constructor(
    ...
    this.recentOrdersTableDataSource = new MatTableDataSource(
      route.snapshot.data["data"].recentOrdersData
    );
    // tslint:disable-next-line:no-string-literal
    this.latestTicketsTableDataSource = new MatTableDataSource(
      route.snapshot.data["data"].latestTicketsData
    );
  ...
  ngOnInit() {
    ...
    // define a custom sort for the date field
    this.latestTicketsTableDataSource.sortingDataAccessor = (
      item,
      property
    ) => {
      switch (property) {
