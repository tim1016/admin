...
@Component({
  ...
  styleUrls: ["./external-filters-table.component.scss"],
  providers: [DatePipe],
})
export class ExternalFiltersTableComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private datePipe: DatePipe) {
    ...
    this.recentOrdersTableDataSource = new MatTableDataSource<RowData>(
      this.originalData
    );
  ...
  applyFilters(form): void {
    const date = form.date
      ? new Date(form.date.year, form.date.month - 1, form.date.day)
      : "";
  ...
  toggleFilters(): void {
    ...
    this.toggleFiltersLabel = this.filtersVisible
      ? "Hide filters"
      : "Show filters";
  }
